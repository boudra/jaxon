defmodule Jaxon.CallbackDecoder do
  alias Jaxon.Decoder
  ...
  defp handle_event(d, module, :start_array, {path, data}) do
    handle_event(d, module, Decoder.decode(d), {[-1 | path], module.insert(data, path, [])})
  end

  defp handle_event(d, module, :end_array, {[key | rest], data}) when is_integer(key) do
    handle_event(d, module, Decoder.decode(d), {rest, module.close(data, rest)})
  end
  ...
  defp handle_event(d, module, :start_object, {path, data}) do
    handle_event(d, module, Decoder.decode(d), {["" | path], module.insert(data, path, %{})})
  end
  ...
  defp handle_event(d, module, {:key, key}, {[_ | path], data}) do
    handle_event(d, module, Decoder.decode(d), {[key | path], data})
  end

  defp handle_event(d, module, {_, value}, {[key | rest], data}) when is_integer(key) do
    new_path = [key + 1 | rest]
    handle_event(d, module, Decoder.decode(d), {new_path, module.insert(data, new_path, value)})
  end
  ...
  defp handle_event(d, module, {_, value}, {path, data}) do
    handle_event(d, module, Decoder.decode(d), {path, module.insert(data, path, value)})
  end
  ...
  def decode(binary, d, module, state) do
    d = Decoder.update(d, binary)
    handle_event(d, module, Decoder.decode(d), state)
  end
end
